...
def main():
    ...
    window = glfwCreateWindow(800, 800, '3-euler-angles-changing', None, None)
    if not window:
        glfwTerminate()
        return
    ...
    while not glfwWindowShouldClose(window):
        ...
        # ZYX Euler angles
        # t = glfwGetTime()
        zang = glm.radians(g_alpha)
        xang = glm.radians(g_beta)
        zang_2 = glm.radians(g_gamma)
        Rz = glm.rotate(zang, (0,0,1))
        Rx = glm.rotate(xang, (1,0,0))
        Rz_2 = glm.rotate(zang_2, (0,0,1))
        M = glm.mat4(Rz * Rx * Rz_2)
